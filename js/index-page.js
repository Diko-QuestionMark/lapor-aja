...
function updateCreateReportAccess(session) {
  const createReportBtn = getById("create-report-btn", "createReportBtn");
  if (!createReportBtn) {
    return;
  }
...
function showToast(message, type) {
  const toastEl = getById("app-toast", "appToast");
  const toastBody = getById("app-toast-body", "appToastBody");
...
function showPhotoError(message) {
  const errorEl = getById("photo-error", "photoError");
  if (!message) {
...
function showTitleError(message) {
  const errorEl = getById("title-error", "titleError");
  if (!errorEl) {
    return;
  }
  if (!message) {
...
function updateSubmitState() {
  const submitBtn = getById("submit-btn", "submitBtn");
  if (!submitBtn) {
    return;
  }
  ...
  const useLocation = getById("use-location", "useLocation").checked;
...
function updatePhotoPreview() {
  const files = getSelectedPhotos();
  const previewWrap = getById("photo-preview-wrap", "photoPreviewWrap");
  const previewGrid = getById("photo-preview-grid", "photoPreviewGrid");
  const previewCount = getById("photo-preview-count", "photoPreviewCount");
...
function getLocation() {
  const locText = getById("location-text", "locText");
  isLocating = true;
  latitude = null;
  longitude = null;
  updateSubmitState();
...
function resetForm() {
  ...
  selectedPhotos = [];
  getById("use-location", "useLocation").checked = false;
  showTitleError("");
  showPhotoError("");
  toggleLocation(getById("use-location", "useLocation"));
  updatePhotoPreview();
}
...
function getSortedReports() {
  const mode = getById("sort-filter", "sortFilter").value;
  const timeMode = getById("time-filter", "timeFilter").value;
  const agencyMode = String(
    (getById("agency-filter-user", "agencyFilterUser") || { value: "all" }).value || "all",
  );
...
function updateSectionTitle() {
  ...
  const timeMode = String(
    (getById("time-filter", "timeFilter") || { value: "all" }).value || "all",
  );
...
function openImageInspect(src, title) {
  const preview = getById("image-inspect-preview", "imageInspectPreview");
  const modalTitle = getById("image-inspect-title", "imageInspectTitle");
  preview.src = src;
...
function initUi() {
  toastInstance = new bootstrap.Toast(getById("app-toast", "appToast"), {
    delay: 2600,
  });
  reportModal = bootstrap.Modal.getOrCreateInstance(
    getById("report-modal", "reportModal"),
  );
  imageInspectModal = bootstrap.Modal.getOrCreateInstance(
    getById("image-inspect-modal", "imageInspectModal"),
  );
  ...
  if (resetFilterBtn) {
    resetFilterBtn.addEventListener("click", function () {
      ...
      const timeFilterInner = getById("time-filter", "timeFilter");
      const agencyFilterInner = getById("agency-filter-user", "agencyFilterUser");
      if (sortFilter) {
  ...
  if (reportModalEl) {
    reportModalEl.addEventListener("hidden.bs.modal", resetForm);
  }
}
